...
def _make_snapshot(
    ...
    envs: dict[str, dict[str, str]] | None = None,
) -> Snapshot:
    """Return a Snapshot with sensible defaults for use in tests."""
    return Snapshot(
        project=project,
        label=label,
        created_at=created_at,
        envs=envs if envs is not None else {"local": {"A": "1"}},
    )
...
def test_restore_snapshot_writes_all_envs():
    global _written
    _written = {}
    snap = _make_snapshot(envs={"local": {"X": "10"}, "prod": {"X": "99"}})
...
def test_restore_snapshot_empty_raises():
    snap = _make_snapshot(envs={})
    with pytest.raises(SnapshotError, match="no envir"):
